...
export interface CreateWalkInAppointmentParams {
  ...
  serviceId: string;
  startTime: string;
  /** Optional: defaults to startTime + the service's own duration, same as every other booking path, unless given directly. */
  endTime?: string;
  customer: {
    name: string;
...
/** GET /appointments' response shape: the bare fields plus lean embedded employee/service/customer names, so a calendar can render without a getAppointmentDetail call per row. */
export interface AppointmentListItem {
  ...
  /** Non-null only for an occurrence the recurring scheduler generated (see the recurring-appointments API); used by the calendar to show a recurring icon without a full detail fetch per row. */
...
export function listAppointments(
  ...
  return authFetch<AppointmentListItem[]>("/appointments", {
    params: {
      employee_id: params.employeeId,
...
export interface AppointmentDetail {
  ...
  /** Present only when this appointment was generated by the recurring scheduler; the series' own weekday/time/interval, enough for a descriptive chip without a separate recurring-appointments fetch. */
